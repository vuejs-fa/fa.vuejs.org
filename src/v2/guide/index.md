...
ویو چند directive دیگر نیز دارد که هر کدام از آنها کارایی منحصر به خود دارند. به عنوان مثال، می‌توانیم از `v-for` برای نمایش یک لیست با استفاده از داده‌های یک متغییر آرایه استفاده کنیم:
...
برای اینکه به کاربران امکان برقراری تعامل با اپلیکیشنمان را بدهیم، می‌توانیم از مشخصه `v-on` که یک Directive است، استفاده کینم تا رویداد خاصی را  تحت نظر گرفته و با وقوع آن یک تابع (method) را در اپلیکیشن ویو فرا‌خوانی کنیم:
...
component system یکی دیگر از مفاهیم با اهمیت در ویو است، چرا که به ما این امکان را می‌دهد که با ترکیب component‌هایی کوچک و مستقل، اپلیکیشن‌هایی در مقیاس بزرگ درست کنیم. این component ها معمولا هدف و کارایی منحصر به خود داشته و امکان استفاده مجدد از آنها در بخش های مختلف یک اپلیکیشن و حتی پروژه های دیگر وجود دارد. اگر خوب در این مورد فکر کنیم، تقریبا واسط کاربری هر اپلیکیشن را می‌توان در یک گراف درختی از component ‌ها تصور کرد:

![Component Tree](/images/components.png)

در ویو، یک component در واقع یک نمونه از شیء Vue با گزینه هایی از پیش تعریف شده می‌باشد.ثبت component ها در ویو خیلی ساده است:
...
حالا می‌توانید از آن در ساخت قالب یک component دیگر استفاده کنید:
...
اما این component برای همه چک لیست‌ها،‌متن یکسانی تولید می‌کند، که خیلی جالب نیست. باید از طریقی داده را از <span title='parent scope'>component والد</span> به <span title='child scope'>component فرزند</span> منتقل کنیم، برای این کار component را ویرایش می‌کنیم تا یک [مشخصه (prop)](components.html#Props) بپذیرد:
...
این یک مثال ساختگی بود، اما ما موفق شدیم که اپلیکیشنمان را به دو بخش کوچکتر تقسیم کنیم و component فرزند به صورت قابل قبولی با استفاده از واسطه props از component والد جداسازی شده است. ما می‌توانیم `<todo-item>` را با منطق و قالبی پیچیده‌تر توسعه دهیم بدون اینکه کاری با اپلیکیشن والد داشته باشیم.

در اپلیکیشن‌های بزرگ لازم است که حتما آن‌ها را به component‌های کوچکتر تقسیم کنیم تا فرایند توسعه اپلیکیشن قابل مدیریت باشد. [بعدا در این راهنما](components.html) خیلی بیشتر در مورد component ها توضیح خواهیم داد، اما در اینجا یک مثال تخیلی را آورده‌ایم که ببینید قالب یک اپلیکیشن با component های مختلف به چه شکل خواهد بود:
...
ممکن است متوجه شده باشید که component های ویو خیلی شبیه به **تگ‌های سفارشی (Custom Elements)** تعریف شده در [Web Components Spec](https://www.w3.org/wiki/WebComponents/) هستند. این به این دلیل است که component های ویو تا حدودی با توجه به این تعاریف طراحی شده ‌اند. برای مثال در component های ویو [Slot API](https://github.com/w3c/webcomponents/blob/gh-pages/proposals/Slots-Proposal.md) و مشخصه مخصوص `is` پیاده سازی شده است. اما چند تفاوت کلیدی نیز وجود دارد:

1. تعاریف Web Components Spec نهایی شده‌اند، اما هنوز در همه مرورگرها پشتیبانی نمی‌شوند. Safari 10.1+، Chrome 54+ و Firefox 63+ از آنها پشیتبانی می‌کنند. در مقابل، component های ویو برای اجرا به هیچ‌گونه افزونه اضافی در مرورگر نیاز نداشته و به خوبی در همه مرورگر‌های پشتیبانی شده (IE9 و بالاتر)‌ کار می‌کنند. در صورت نیاز component های ویو را می‌توان همچنین درون تگ‌های سفارشی نیز قرار داد.

2. component های ویو امکانات ویژه‌ای را فراهم ‌می‌کنند که در تگ‌های سفارشی وجود ندارند، به عنوان مثال جریان انتقال داده‌ها میان component ها، تعامل رویدادی و یکپارچگی تولیدابزار.

اگرچه ویو به صورتی داخلی از تگ‌های سفارشی استفاده نمی‌کند، وقتی که قرار باشدبه عنوان تگ سفارشی استفاده یا پخش شود از  [قابلیت هماهنگی بالایی](https://custom-elements-everywhere.com/#vue) برخوردار است. همچنین Vue CLI از ساخت component هایی که خود را به عنوان تگ‌های سفارشی ثبت می‌کنند، پشتیبانی می‌کند.
